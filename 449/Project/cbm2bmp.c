...
void convertCBMtoBMP(const char *cbmFilename, const char *bmpFilename)
{
    ...
    int tempWidth = cbmHeader.width * 3;
    int pad = 0;
    while ((tempWidth % 4) != 0)
    {
        tempWidth++;
    ...
    uint32_t bmpImageSize = (cbmHeader.width * cbmHeader.height * 3) + sizeof(DIBHeader) + sizeof(BMPHeader) + (pad * cbmHeader.height);

    // calculate the size of bmp
    uint32_t bmpSize = sizeof(BMPHeader) + sizeof(DIBHeader);

    // update the bmp and dib headers
    bmpHeader.format[0] = 'B';
    bmpHeader.format[1] = 'M';
    bmpHeader.size = bmpImageSize;
    bmpHeader.reserved1 = 0;
    bmpHeader.reserved2 = 0;
    bmpHeader.offset = bmpSize;
